...
void dirMixQuartersLayer( unsigned char * block, unsigned long long blocklen, unsigned char * state );
...
void inc32( unsigned char * block, unsigned long long blocklen, unsigned inc );
...
void sumAD( struct FlexAEADv1 * self, unsigned char * ADblock, int doublePFK );
...
inline void inc32( unsigned char * block, unsigned long long blocklen, unsigned inc )
{
...
inline void dirMixQuartersLayer( unsigned char * block, unsigned long long blocklen, unsigned char * state )
{
    /*
	unsigned long long i = 0;
	for( i=0; i<(blocklen/4); i++)
	{
        *(state+3*(blocklen/4)+i) = *(block+0*(blocklen/4)+i) ^
                                    *(block+1*(blocklen/4)+i) ^
                                    *(block+2*(blocklen/4)+i) ^
                                    *(block+3*(blocklen/4)+i);
        *(state+0*(blocklen/4)+i) = *(state+3*(blocklen/4)+i) ^
                                    *(block+0*(blocklen/4)+i);
        *(state+1*(blocklen/4)+i) = *(state+3*(blocklen/4)+i) ^
                                    *(block+1*(blocklen/4)+i);
        *(state+2*(blocklen/4)+i) = *(state+3*(blocklen/4)+i) ^
                                    *(block+2*(blocklen/4)+i);
        *(state+3*(blocklen/4)+i) = *(state+3*(blocklen/4)+i) ^
                                    *(block+3*(blocklen/4)+i);
	}
    */
    switch(blocklen)
...
void encryptBlock( struct FlexAEADv1 * self, unsigned char * block )
{
    ...
    *(((crypto_uint64 *) block)+0) ^= *(((crypto_uint64 *) (*self).sn)+0);
#if BLOCKSIZE >= 16  
    *(((crypto_uint64 *) block)+1) ^= *(((crypto_uint64 *) (*self).sn)+1);
#endif    
#if BLOCKSIZE == 32  
    *(((crypto_uint64 *) block)+2) ^= *(((crypto_uint64 *) (*self).sn)+2);
    *(((crypto_uint64 *) block)+3) ^= *(((crypto_uint64 *) (*self).sn)+3);
#endif    
